            ...
            for y in (-1, 0, 1):
                for x in (-1, 0, 1):
                    ...
                    if 0 <= ny < alto and 0 <= nx < ancho:
                        if matriz[ny][nx] < minimo:
                            minimo = matriz[ny][nx]
            resultado[i][j] = minimo
    return resultado

def erosion(imagen:bytes, ancho:int, alto:int, size:int) -> bytes:
    if size < 1:
        raise ValueError("El tamaño de la erosión debe ser  mayor  o igual 1")
...
def main():
    ...
    output_pixeles = erosion(imagen, ancho, alto, size)
